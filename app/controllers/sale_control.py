...
@router.get("/", response_model=list[SaleResponse], summary="Get All Sales", description="Returns all sales.")
def get_all_sales(
    service: SaleService = Depends(get_sale_service),
    current_user=Depends(require_authenticated_user),
):
...
@router.get(
    "/{sale_id}", response_model=SaleResponse, summary="Get Sale", description="Returns a sale using its ID."
)
def get_sale(
    sale_id: int,
    service: SaleService = Depends(get_sale_service),
